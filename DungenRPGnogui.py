...
class stats:
    playerlocation = player_max_health = player_experiance = player_level = player_gold = None
    def __new__(cls,pl = None, pmh = None, pxp =None, plevel = None, pg = None):
...
class game:
    ...
    def streets(self):
        print('level:',self.level,'health:',self.max_health,'damage:',self.dmg)
        usr = input()
        for i in range(2):
...
class menu:
    ...
    def switch_logic(usr):
        for i in range(3):
            if usr.lower() == ['load','clear','quit','start'][i]:[menu.load,menu.clear,menu.quit,game.start_default][i]()
